...
def run_multiprocess(full_data, dimension, numProcesses=2, threshold=1):
    ...
    if __name__ == '__main__':
        ...
        result_list = pool.starmap(iterate_RMSs, zip(list_of_combinations, repeat(full_data.copy()), repeat(threshold)))
...
parser.add_argument('-i','--input',
    help='Input file name as .csv. Compounds in columns, mutants in rows.', required=True)
parser.add_argument('-o','--output',
    help='Output file name as .csv', required=True)
parser.add_argument('-d','--dimension',
    help='Dimension of search. Default: 2.', default=2, type=int)
parser.add_argument('-p','--processes',
    help='Number of processes to spawn. Default: 1.', default=1, type=int)
parser.add_argument('-l','--length',
    help='Length of result list to print to csv. default: 1000.', default=1000, type=int)
parser.add_argument('-t','--threshold',
    help='Number below which RMSs should be kept. Setting to a lower number (0.15) reduces memory usage for larger dimension searches. default: 1.', default=1, type=float)
args = parser.parse_args()

# run the script printing start and end times and the top five hits at the end.
print('Running a {}x{} matrix search on {} with {} process(es). Threshold set to {}.'.format(args.dimension, args.dimension, args.input, args.processes, args.threshold))
try:
...
# Start the algorithm.
result = run_multiprocess(full_data, args.dimension, numProcesses = args.processes, threshold = args.threshold)
