...
class Model():
    ...
    def run_CC(self):
        self.resultList = run_multiprocess(
            self.df, self.dimension, numProcesses=self.processes, seq_addition=self.seq_addition)
        print('Done!: {}'.format(self.resultList[:10]))
        return self.resultList

    def format_result(self):
        return format_OSF(self.resultList, self.df)
...
class View():
    ...
    def __init__(self, master):
        ...
        # Area for save and close buttons TODO add a frame here to house these
        # buttons
...
class Controller():
    ...
    def saveResult(self, event):
        if self.model.resultList is None:
            ...
            try:
                self.model.format_result().to_csv(savePath, index=False)
            except:
                messagebox.showerror(
